...
from countries import get_shape_polygon, Countires
from fit import fit_function
from config import SAMPLES_PER_GENERATION, TAKE_TOP, NORMAL_MUTATIONS_STABILITY_RATE, BIG_MUTATIONS_STABILITY_RATE, HUGE_MUTATIONS_STABILITY_RATE
import numpy as np
...
class Arena:
    def __init__(self):
        ...
        selected = coutnries.get_all_countires(self.target_area)
    ...
    def pass_one_generation(self):
        ...
        print("Generation Time", int(after_mill - init_mill), " ms")

    def __mutate(self):
        shape = self.samples.shape
        mutations = np.random.randint( -10 *  NORMAL_MUTATIONS_STABILITY_RATE, 2, size=(shape[0], shape[1], shape[3]))
        mutations =  np.where(mutations < -2, 0, np.clip(mutations, -2, 2))

        big_mutations = np.random.randint(-100 * BIG_MUTATIONS_STABILITY_RATE, 40, size=(shape[0], shape[1], shape[3]))
        big_mutations =  np.where(big_mutations < -40, 0, np.clip(big_mutations, -40, 40))

        hug_mutation = np.random.randint(-100 * HUGE_MUTATIONS_STABILITY_RATE, 100, size=(shape[0], shape[1], shape[3]))
